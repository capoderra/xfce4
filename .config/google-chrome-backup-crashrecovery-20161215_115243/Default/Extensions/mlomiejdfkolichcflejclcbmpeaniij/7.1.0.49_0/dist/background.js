...
},{"./classes/Conf":15,"./common":29,"./globals":30,"./utils":33,"underscore":7}],9:[function(require,module,exports){
'use strict';
...
var _i18n = require('./classes/i18n');
...
var _TagDb = require('./classes/TagDb');

var _TagDb2 = _interopRequireDefault(_TagDb);
...
window.i18n = _i18n2.default;
window.DEBUG = _globals2.default.DEBUG;

function raiseSettingsFlag() {
	FROM_SETTINGS = true;
	setTimeout(() => {
		FROM_SETTINGS = false;
	}, 500);
...
function checkLibraryVersion() {
	return new Promise(function (resolve, reject) {
		...
		utils.getJson(VERSION_CHECK_URL).then(data => {
			...
			_TagDb2.default.update(data.tagsVersion);
...
function getPanelDataHelper(tab) {
	...
	models.push({
		...
		panel_expanded: _Conf2.default.panel_expanded || IS_FIREFOX || IS_EDGE || false,
...
function handleSettingsPage(name, message, tab_id, callback) {
	...
	if (name === "getPageSettings") {
		...
		return false;
	} else if (name === 'update_language') {
		FROM_SETTINGS = false;
		_Conf2.default.language = message.language;

		_i18n2.default.init(_Conf2.default.language).then(function () {
			accounts.pushUserSettings(extensionweb.getSettings());
			callback(extensionweb.getTranslations());
		}).catch(function (err) {
			log('handleSettingsPage update_language error', err);
		});
		return true;
		...
		checkLibraryVersion().then(function (result) {
			...
			if (IS_EDGE) {
				sendMessage(tab_id, 'database_updated', settings);
			} else {
				callback(settings);
			}
		}).catch(function (err) {
			if (IS_EDGE) {
				sendMessage(tab_id, 'database_updated', {});
			} else {
				callback();
			}
	...
	if (name === 'dismissCMPMessage') {
		...
		try {
			...
			updatePanelData(data).then(result => {
				utils.getActiveTab(tab => {
					...
					sendMessage(tabId, 'onFileImported', {
						type: 'message',
						text: _i18n2.default.t("settings_import_success") + " " + (0, _momentWithLocalesMin2.default)(data.settings_last_imported).format('LLL')
					});
				});
			});
		} catch (err) {
			utils.getActiveTab(tab => {
				...
				sendMessage(tabId, 'onFileImported', {
					type: 'error',
					text: _i18n2.default.t("settings_import_file_error")
				});
			});
		}
	}
...
function handleBlockedRedirect(name, message, tab_id, callback) {
	if (name === 'getBlockedRedirectData') {
		if (IS_EDGE) {
			sendMessage(tab_id, 'blockedRedirectData', BLOCKED_REDIRECT_DATA);
		} else {
...
function onMessageHandler(name, message, origin, tab_id, tab_url, callback) {
	if (origin === 'intro') {
	...
	if (name === 'getPanelData') {
		...
		utils.getActiveTab(tab => {
			if (tab && tab.id && tab.url.startsWith('http')) {
				injectNotifications(tab.id, true).then(result => {
					if (result) {
						sendMessage(tab.id, 'showBrowseWindow', {
							translations: {
								browse_button_label: _i18n2.default.t('browse_button_label'),
								select_file_for_import: _i18n2.default.t('select_file_for_import'),
								file_was_not_selected: _i18n2.default.t('file_was_not_selected') }
						}, result => {
							if (chrome.runtime.lastError) {
								callback(_i18n2.default.t('refresh_and_try_again'));
							} else {
								callback();
							}
						});
				...
				callback(_i18n2.default.t('not_http_page'));
			}
		});
		return true;
	}
	...
	Promise.all(promises).catch(err => {
		...
		c2pApp.forEach(function (c2pAppDef) {
			const tplData = {
				...
				allow_always_title: _i18n2.default.t('click2play_allow_always_tooltip')
			};

			if (c2pAppDef.button) {
				tplData.allow_once_title = _i18n2.default.t('click2play_allow_once_button_tooltip', app_name);
				let url = 'app/images/click2play/' + c2pAppDef.button;
				url = url.replace('.png', '.data');
				const conf_prop = url.slice(url.lastIndexOf('/') + 1).replace('.', '_');
				tplData.allow_once_src = _Conf2.default[conf_prop];
			} else {
				tplData.allow_once_title = _i18n2.default.t('click2play_allow_once_tooltip');
				tplData.allow_once_src = _Conf2.default.allow_once_data;

				tplData.ghostery_blocked_title = _i18n2.default.t('click2play_blocked', app_name);

				if (c2pAppDef.type) {
					tplData.click2play_text = _i18n2.default.t('click2play_' + c2pAppDef.type + '_form', app_name);
				}
			}
...
function buildRedirectCP2(requestId, redirectUrls, app_id) {
	...
	BLOCKED_REDIRECT_DATA.translations = {
		blocked_redirect_page_title: _i18n2.default.t('blocked_redirect_page_title'),
		blocked_redirect_prevent: _i18n2.default.t('blocked_redirect_prevent', utils.processUrl(redirectUrls.url).host, utils.processUrl(redirectUrls.redirectUrl).host, app_name, 'http://www.ghostery.com/apps/' + encodeURIComponent(app_name.replace(/\s+/g, '_').toLowerCase())),

		blocked_redirect_action_always_title: _i18n2.default.t('blocked_redirect_action_always_title'),
		blocked_redirect_action_through_once_title: _i18n2.default.t('blocked_redirect_action_through_once_title'),
		blocked_redirect_url_content: _i18n2.default.t('blocked_redirect_url_content', redirectUrls.redirectUrl, app_name)
	};
	...
	setTimeout(function () {
		...
		if (block && _Conf2.default.ghostrank === 1) {
			utils.getTab(tab_id, function (tab) {
				...
				ghostrank.recordStats(tab.url, details.url, bug_id, true, -1, -1, -1, -1, _TabInfo2.default.getTabInfo(tab_id, 'timestamp'), -1, -1);
			});
		}
	}, 1);
				...
				if (result) {
					sendMessage(tab_id, 'showUpgradeAlert', {
						translations: _underscore2.default.object(_underscore2.default.map(alert_messages, function (key) {
							return [key, _i18n2.default.t(key)];
						})),
						language: _Conf2.default.language
					});
				}
...
function initializeDispatcher() {
	...
	_Dispatcher2.default.on('conf.save.language', function (lang) {
		_i18n2.default.init(lang).then(() => {
			utils.sendMessageToPanel('onLanguageUpdated');
			utils.broadcastMessage('onLanguageUpdated', lang);
		}).catch(function (err) {
			log('initializeDispatcher conf.save.language error', err);
		});
	});
	...
	_Dispatcher2.default.on('conf.save.panel_expanded', function (expanded) {
		chrome.browserAction.setPopup({
			popup: expanded || IS_FIREFOX || IS_EDGE ? 'app/panel_expanded.html' : 'app/panel.html'
		});
	});
	_Dispatcher2.default.on('conf.changed.settings', _underscore2.default.debounce(key => {
		...
		if (settings && settings.conf) {
			...
			settings.conf.ghostrank = settings.conf.ghostrank === 2 ? 0 : settings.conf.ghostrank;
			settings.conf.account_dismissed = true;
...
function initializePopup() {
	chrome.browserAction.setPopup({
		popup: _Conf2.default.panel_expanded || IS_FIREFOX || IS_EDGE ? 'app/panel_expanded.html' : 'app/panel.html'
	});
}
...
function initializeVersioning() {
	...
	if (!previousVersion) {
		...
		previousVersion = localStorage && localStorage.getItem('previousVersion');
		...
		log("UPGRADE FROM CHROME OR OPERA < 7.0. PREVIOUS VERSION IS:", previousVersion);
		JUST_UPGRADED = true;
...
function initializeGhosteryModules() {
	...
	return Promise.all([_BugDb2.default.init(JUST_UPGRADED), _Click2PlayDb2.default.init(JUST_UPGRADED), _CompatibilityDb2.default.init(JUST_UPGRADED), _TagDb2.default.init(JUST_UPGRADED), _SurrogateDb2.default.init(JUST_UPGRADED)]);
}
...
function initializeEventListeners() {
	...
	onMessage.addListener(function (request, sender, callback) {
		...
		if (!onMessageHandler(name, message, origin, tab_id, tab_url, callback)) {
			callback();
		}
	...
	if (typeof chrome.runtime.setUninstallURL === 'function') {
		var uninstallURL = (0, _metrics.getUninstallURL)();
		if (uninstallURL && uninstallURL.length) {
...
function init() {
	_Conf2.default.init().then(language => {
		return _i18n2.default.init(language).then(() => {
			initializePopup();
...
},{"../app/templates/precompiled/click2play":1,"./accounts":8,"./classes/BugDb":10,"./classes/Button":11,"./classes/CMP":12,"./classes/Click2PlayDb":13,"./classes/CompatibilityDb":14,"./classes/Conf":15,"./classes/Dispatcher":16,"./classes/ExtensionWeb":17,"./classes/FoundBugs":18,"./classes/Ghostrank":19,"./classes/Latency":20,"./classes/Policy":21,"./classes/PurpleBox":22,"./classes/SurrogateDb":23,"./classes/TabInfo":24,"./classes/TagDb":25,"./classes/TrackerMap":26,"./classes/i18n":28,"./common":29,"./globals":30,"./matcher":31,"./metrics":32,"./utils":33,"moment/min/moment-with-locales.min.js":6,"underscore":7}],10:[function(require,module,exports){
'use strict';
...
},{"../common":29,"../utils":33,"./Conf":15,"./Updatable":27,"underscore":7}],11:[function(require,module,exports){
'use strict';
				...
				(0, _utils.getTab)(tabId, () => {
					if (typeof chrome.browserAction.setTitle === 'function') {
						chrome.browserAction.setTitle({
							title: _i18n2.default.t("browser_button_tooltip"),
							tabId: tabId
						});
					}
...
},{"../common":29,"../utils":33,"./Conf":15,"./FoundBugs":18,"./Policy":21,"./i18n":28}],12:[function(require,module,exports){
'use strict';
...
},{"../common":29,"../globals":30,"../utils":33,"./Conf":15}],13:[function(require,module,exports){
'use strict';
...
},{"../common":29,"./Conf":15,"./Updatable":27,"underscore":7}],14:[function(require,module,exports){
'use strict';
...
},{"../common":29,"../matcher":31,"./Conf":15,"./Updatable":27}],15:[function(require,module,exports){
'use strict';
...
class Conf {
	...
	init() {
		return (0, _common.prefsGet)('alert_bubble_pos', 'alert_bubble_timeout', 'ignore_first_party', 'enable_autoupdate', 'ghostrank', 'local_messages', 'account_dismissed', 'upgrade_dismissed', 'tour_alert_dismissed', 'panel_expanded', 'show_alert', 'hide_alert_trusted', 'show_badge', 'alert_expanded', 'block_by_default', 'notify_library_updates', 'notify_upgrade_updates', 'notify_hotfix_updates', 'enable_metrics', 'enable_click2play', 'enable_click2play_social', 'db_last_updated', 'settings_last_imported', 'settings_last_exported', 'show_cmp', 'show_tracker_urls', 'toggle_individual_trackers', 'bugs_last_updated', 'bugs_last_checked', 'import_callout_dismissed', 'previous_version', 'previousVersion', 'install_random_number', 'install_date', 'last_cmp_date', 'cmp_version', 'global_settings_updated', 'metrics', 'cmp_data', 'bugs', 'click2play', 'compatibility', 'surrogates', 'tags', 'new_app_ids', 'selected_app_ids', 'site_whitelist', 'site_blacklist', 'language', 'site_specific_unblocks', 'site_specific_blocks', 'paused_blocking', 'global_settings', 'cta_status', 'login_info', 'reload_banner_status', 'trackers_banner_status').then(data => {
			...
			_initProperty('panel_expanded', true);
			...
			_initProperty('language', this._getDefaultLanguage());
			...
			(0, _common.pref)('paused_blocking', false);
	...
	_getDefaultLanguage() {
		...
		if (_i18n2.default.SUPPORTED_LANGUAGES.hasOwnProperty(lang)) {
			return lang;
		}
...
var handler = {
	...
	set: function (target, key, value) {
		...
		if (!target.onInit) {
			...
			if (target.SYNC_SET.has(key) && key !== 'language') {
...
},{"../common":29,"../globals":30,"../utils":33,"./Dispatcher":16,"./i18n":28,"underscore":7}],16:[function(require,module,exports){
'use strict';
...
},{"../common":29}],17:[function(require,module,exports){
'use strict';
...
class ExtensionWeb {
	...
	getTranslations() {
		return {
			library_updated_on: _i18n2.default.t('library_updated_on'),
			library_never_updated: _i18n2.default.t('library_never_updated'),
			library_update_now_link: _i18n2.default.t('library_update_now_link'),
			library_update_successful: _i18n2.default.t('library_update_successful'),
			library_update_failed: _i18n2.default.t('library_update_failed'),
			library_update_already_updated: _i18n2.default.t('library_update_already_updated'),
			library_update_in_progress: _i18n2.default.t('library_update_in_progress'),
			whitelist_error_duplicate_url: _i18n2.default.t('whitelist_error_duplicate_url'),
			whitelist_error_blacklist_url: _i18n2.default.t('whitelist_error_blacklist_url'),
			blacklist_error_duplicate_url: _i18n2.default.t('blacklist_error_duplicate_url'),
			blacklist_error_whitelist_url: _i18n2.default.t('blacklist_error_whitelist_url'),
			white_black_list_error_invalid_url: _i18n2.default.t('white_black_list_error_invalid_url'),
			category_uncategorized: _i18n2.default.t('category_uncategorized'),
			category_advertising: _i18n2.default.t('category_advertising'),
			category_comments: _i18n2.default.t('category_comments'),
			category_customer_interaction: _i18n2.default.t('category_customer_interaction'),
			category_essential: _i18n2.default.t('category_essential'),
			category_pornvertising: _i18n2.default.t('category_pornvertising'),
			category_site_analytics: _i18n2.default.t('category_site_analytics'),
			category_social_media: _i18n2.default.t('category_social_media'),
			category_audio_video_player: _i18n2.default.t('category_audio_video_player'),
			category_uncategorized_desc: _i18n2.default.t('category_uncategorized_desc'),
			category_advertising_desc: _i18n2.default.t('category_advertising_desc'),
			category_comments_desc: _i18n2.default.t('category_comments_desc'),
			category_customer_interaction_desc: _i18n2.default.t('category_customer_interaction_desc'),
			category_essential_desc: _i18n2.default.t('category_essential_desc'),
			category_pornvertising_desc: _i18n2.default.t('category_pornvertising_desc'),
			category_site_analytics_desc: _i18n2.default.t('category_site_analytics_desc'),
			category_social_media_desc: _i18n2.default.t('category_social_media_desc'),
			category_audio_video_player_desc: _i18n2.default.t('category_audio_video_player_desc'),
			tracker_description_getting: _i18n2.default.t('tracker_description_getting'),
			tracker_description_none_found: _i18n2.default.t('tracker_description_none_found'),
			tracker_description_learn_more: _i18n2.default.t('tracker_description_learn_more'),
			blocking_options_trackers: _i18n2.default.t('blocking_options_trackers'),
			blocking_options_blocked: _i18n2.default.t('blocking_options_blocked'),
			blocking_options_blocked_all: _i18n2.default.t('blocking_options_blocked_all')
		};
	}
}
...
},{"../accounts":8,"../common":29,"../globals":30,"./BugDb":10,"./Conf":15,"./i18n":28,"underscore":7}],18:[function(require,module,exports){
'use strict';
		...
		for (id in bugs) {
			...
			if (apps_obj.hasOwnProperty(aid)) {
				...
				apps_obj[aid] = {
					...
					tags: this._mapTags(db.apps[aid].tags),
	...
	_mapTags(tag_array) {
		...
		for (var i = 0; i < tag_array.length; i++) {
			if (typeof tag_array[i] === "number") {
				output_array.push(_TagDb2.default.db.list[tag_array[i]].name);
			}
		}
		return output_array;
	}
...
},{"../common":29,"./BugDb":10,"./CompatibilityDb":14,"./TabInfo":24,"./TagDb":25}],19:[function(require,module,exports){
'use strict';
...
},{"../common":29,"../globals":30,"../utils":33,"./BugDb":10,"./Conf":15,"underscore":7}],20:[function(require,module,exports){
'use strict';
...
},{"../utils":33,"./Conf":15,"./FoundBugs":18,"./Ghostrank":19,"./TabInfo":24,"underscore":7}],21:[function(require,module,exports){
'use strict';
...
},{"./Click2PlayDb":13,"./Conf":15}],22:[function(require,module,exports){
'use strict';
		...
		return (0, _utils.injectScript)(tab_id, 'dist/purplebox.js', 'dist/css/purplebox.css', 'document_start').then(() => {
			...
			return (0, _accounts.getLoginInfo)(true).then(result => {
				this._sendMessage(tab_id, 'createBox', {
					...
					translations: {
						looking: _i18n2.default.t('box_looking'),
						trackers: _i18n2.default.t('box_trackers'),
						box_warning_compatibility: _i18n2.default.t('box_warning_compatibility'),
						box_warning_slow: _i18n2.default.t('box_warning_slow'),
						box_warning_nonsecure: _i18n2.default.t('box_warning_nonsecure'),
						tracker: _i18n2.default.t('box_tracker'),
						hide: _i18n2.default.t('box_hide'),
						settings: _i18n2.default.t('box_settings'),
						options_expanded: _i18n2.default.t('box_options_expanded'),
						hide_expanded: _i18n2.default.t('box_hide_expanded'),
						settings_expanded: _i18n2.default.t('box_settings_expanded'),
						box_dismiss_0s: _i18n2.default.t('box_dismiss_0s'),
						box_dismiss_5s: _i18n2.default.t('box_dismiss_5s'),
						box_dismiss_15s: _i18n2.default.t('box_dismiss_15s'),
						box_dismiss_30s: _i18n2.default.t('box_dismiss_30s'),
						box_display_br: _i18n2.default.t('box_display_br'),
						box_display_tr: _i18n2.default.t('box_display_tr'),
						box_display_tl: _i18n2.default.t('box_display_tl'),
						box_display_bl: _i18n2.default.t('box_display_bl')
					},
...
},{"../accounts":8,"../common":29,"../globals":30,"../utils":33,"./Conf":15,"./FoundBugs":18,"./Policy":21,"./TabInfo":24,"./i18n":28}],23:[function(require,module,exports){
'use strict';
...
},{"../common":29,"./Conf":15,"./Updatable":27,"underscore":7}],24:[function(require,module,exports){
'use strict';
...
class TagDb extends _Updatable2.default {
	buildDb(tags, version) {
		var map = {};
	...
	processList(data) {
		...
		(0, _common.log)('processing tags...');

		try {
			db = this.buildDb(data.tags, data.tagsVersion);
		} catch (e) {
			(0, _common.log)('TagDb processList() error', e);
			return false;
		}
		...
		(0, _common.log)('processed tags...');

		this.db = db;
		_Conf2.default.tags = data;

		return true;
	}
}

exports.default = new TagDb('tags');
module.exports = exports['default'];

},{"../common":29,"./Conf":15,"./Updatable":27}],26:[function(require,module,exports){
'use strict';
...
},{"../common":29,"../globals":30,"../utils":33,"./Conf":15}],27:[function(require,module,exports){
'use strict';
...
},{"../common":29,"../globals":30,"../utils":33,"./Conf":15,"underscore":7}],28:[function(require,module,exports){
'use strict';
...
const DEBUG = _globals2.default.DEBUG;
const EXTENSION_VERSION = _globals2.default.EXTENSION_VERSION;
...
class i18n {
	...
	init(lang) {
		var langToken = lang && this.SUPPORTED_LANGUAGES.hasOwnProperty(lang) ? lang : 'en';
		return this._getMessagesLocal('en').then(data => {
			if (langToken === 'en') {
				_Conf2.default.local_messages.lang = langToken;
				_Conf2.default.local_messages.version = EXTENSION_VERSION;
				_Conf2.default.local_messages.messages = data;
				return data;
			} else {
				return this._getMessagesLocal(langToken).then(() => {
					_Conf2.default.local_messages.lang = langToken;
					_Conf2.default.local_messages.version = EXTENSION_VERSION;
				});
			}
	...
	t(...args) {
		var local_messages = _Conf2.default.local_messages || {};
		var translation = local_messages.messages[args[0]];

		if (!translation && args[0] !== 'en') {
			translation = local_messages.messagesEn[args[0]];
		}

		if (!translation) {
			(0, _common.log)("NO TRANSLATIONS FOR", args[0], local_messages.messages, local_messages.messagesEn);
			return args[0];
		}

		var placeholders = translation && translation.placeholders || {},
		    substitutions = _underscore2.default.flatten(args.slice(1)),
		...
		for (var name in placeholders) {
			...
			msg = msg.replace(new RegExp('\\$' + name + '\\$', 'gi'), placeholders[name].content.replace(/\$\d/g, substitutor));
		}
		return msg;
	}

	_getMessagesLocal(lang) {
		var local_messages = _Conf2.default.local_messages || {};
		return new Promise((resolve, reject) => {
			if ((lang === 'en' && local_messages.messagesEn || lang !== 'en' && local_messages.lang === lang && local_messages.messages) && local_messages.version === EXTENSION_VERSION && !DEBUG) {
				resolve(lang === 'en' ? local_messages.messagesEn : local_messages.messages);
			} else {
				this._getMessages(lang).then(data => {
					resolve(data);
				}).catch(err => {
					(0, _common.log)('_getMessagesLocal Error', err);
					reject(err);
				});
			}
		});
	...
	_getMessages(lang) {
		return (0, _utils.getJson)(chrome.extension.getURL('_locales/' + lang + '/messages.json')).then(result => {
			if (lang === 'en') {
				_Conf2.default.local_messages.messagesEn = result;
			} else {
				_Conf2.default.local_messages.messages = result;
			}
			return result;
		}).catch(function (err) {
			(0, _common.log)('i18n _getMessages error', err);
			return Promise.reject(new Error('i18n _getMessages error', err));
		});
	}
}
...
},{"../common.js":29,"../globals":30,"../utils.js":33,"./Conf":15,"underscore":7}],29:[function(require,module,exports){
(function (Buffer){
'use strict';
...
},{"./classes/BugDb":10,"./classes/Conf":15,"./common":29,"./utils":33}],32:[function(require,module,exports){
'use strict';
...
exports.ping = ping;
exports.getUninstallURL = getUninstallURL;
...
const FREQUENCIES = {
	...
	monthly: 2419200000
      ...
      EXTENSION_VERSION = _globals2.default.EXTENSION_VERSION,
      CRITICAL_METRICS = ['uninstall', 'install_success', 'install', 'upgrade', 'active', 'engaged'],
      BROWSER_INFO = (0, _common.browserInfo)();
	...
	frequencies.forEach(frequency => {
		if (_checkPing(type, frequency)) {
			let timeNow = Number(new Date().getTime()),
			    frequencyString = type !== 'uninstall' ? '/' + frequency : '';

			_Conf2.default.metrics[type + '_' + frequency] = timeNow;

			metrics_url = 'https://' + METRICS_SUB_DOMAIN + '.ghostery.com/' + type + frequencyString + '?gr=' + _Conf2.default.ghostrank + '&v=' + encodeURIComponent(EXTENSION_VERSION) + '&install_rand=' + encodeURIComponent(_Conf2.default.install_random_number) + '&signed_in=' + (_Conf2.default.login_info.logged_in ? '1' : '0') + '&install_date=' + encodeURIComponent(_Conf2.default.install_date) + '&noncritical=' + (_Conf2.default.enable_metrics ? '1' : '0') + '&purplebox=' + (_Conf2.default.show_alert ? _Conf2.default.alert_expanded ? '1' : '2' : '0') + '&panel_expanded=' + (_Conf2.default.panel_expanded ? '1' : '0') + '&show_cmp=' + (_Conf2.default.show_cmp ? '1' : '0') + '&ua=' + encodeURIComponent(BROWSER_INFO.token) + '&os=' + encodeURIComponent(BROWSER_INFO.os) + '&l=' + encodeURIComponent(_Conf2.default.language);
...
},{"./classes/Conf":15,"./common":29,"./globals":30}],33:[function(require,module,exports){
'use strict';
...
},{"./classes/FoundBugs":18,"./classes/TabInfo":24,"./common":29,"underscore":7}]},{},[9])
